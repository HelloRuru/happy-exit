...
function App() {
  ...
  const defaultChecklist: ChecklistItem[] = [
    { id: 'c1', text: '提交書面離職信', checked: false, category: '法律權益', priority: 'must' },
    ...
    { id: 'c4', text: '索取服務證明書（無不利記載）', checked: false, category: '法律權益', priority: 'must', note: '依勞基法§19' },
    ...
    { id: 'c8', text: '檢查勞退6%最後提繳紀錄', checked: false, category: '法律權益', priority: 'must', note: '至勞保局e化服務查詢' },
    ...
    { id: 'c11', text: '使用謀職假（預告期每週2日有薪）', checked: false, category: '法律權益', priority: 'should' },
    ...
    { id: 'c14', text: '撰寫交接文件', checked: false, category: '工作交接', priority: 'must' },
    { id: 'c15', text: '與接手同事交接會議', checked: false, category: '工作交接', priority: 'must' },
    ...
    { id: 'c29', text: '發送Farewell Email', checked: false, category: '人際關係', priority: 'should' },
  ...
  const reset = () => { if(confirm('確定重新開始？')) { localStorage.removeItem('resignV3'); location.reload(); } };
  ...
  const exportHTML = (items: ChecklistItem[]) => {
    ...
    const html = `<!DOCTYPE html><html lang="zh-TW"><head><meta charset="UTF-8"><title>離職清單</title><style>body{font-family:sans-serif;max-width:800px;margin:40px auto;padding:20px}h1{color:#6B8CAE}h2{margin-top:30px}.item{padding:8px 0;border-bottom:1px solid #eee;display:flex;gap:10px}.cb{width:18px;height:18px;border:2px solid #6B8CAE;border-radius:4px}.checked{background:#6B8CAE}.note{font-size:12px;color:#888}</style></head><body><h1>📋 離職交接清單</h1><p>產生日期：${new Date().toLocaleDateString('zh-TW')}</p>${cats.map(c=>`<h2>${c}</h2>${items.filter(i=>i.category===c).map(i=>`<div class="item"><div class="cb ${i.checked?'checked':''}"></div><div><div>${i.text}</div>${i.note?`<div class="note">💡 ${i.note}</div>`:''}</div></div>`).join('')}`).join('')}</body></html>`;
